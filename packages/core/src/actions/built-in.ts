import type { InspectedElement, InspektAction, InspektCustomEditor } from '../types.js';
...
const AGENT_ICON = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m22 2-7 20-4-9-9-4Z"/><path d="M22 2 11 13"/></svg>`;
...
export function createSendToAgentAction(opts: SendToAgentOptions): InspektAction {
  return {
    id: 'send-to-agent',
    label: 'Send to Agent',
    icon: AGENT_ICON,
    async handler(element: InspectedElement) {
      try {
        const url = `${opts.daemonUrl.replace(/\/$/, '')}/__inspekt/grab`;
        const body = JSON.stringify({
          ...
          element: {
            ...
            id: element.id,
            ...(element.snippet ? { snippet: element.snippet } : {}),
          },
          source: 'extension',
        });
        const res = await fetch(url, {
          ...
          headers: {
            ...
            'X-Inspekt-Token': opts.token,
          },
          body,
        });
        if (!res.ok) throw new Error(`Daemon returned ${res.status}`);
        const grab = (await res.json()) as { id: string };
        opts.onSent?.(grab.id);
      } catch (err) {
