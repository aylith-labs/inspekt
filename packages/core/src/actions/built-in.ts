...
export function createOpenEditorAction(serverUrl: string, editor: string): DevLensAction {
  return {
    id: 'open-editor',
    label: 'Open in Editor',
    icon: EDITOR_ICON,
    handler(element: InspectedElement) {
      const url = `${serverUrl}/__devlens/open`;
...
export function createCopyPathAction(): DevLensAction {
  return {
    id: 'copy-path',
    label: 'Copy Path',
    icon: COPY_ICON,
...
export function createOpenGithubAction(repo: string, branch: string): DevLensAction {
  return {
    id: 'open-github',
...
export function createConsoleLogAction(): DevLensAction {
  return {
    id: 'console-log',
    label: 'Log to Console',
    icon: CONSOLE_ICON,
    handler(element: InspectedElement) {
      console.group(`[DevLens] ${element.componentName}`);
