...
export class TreePanel {
  ...
  constructor(
    ...
    this.container.className = `devlens-tree-panel devlens-tree-${options.position}`;
  ...
  private buildUI(): void {
    ...
    header.className = 'devlens-tree-header';

    const title = document.createElement('span');
    title.className = 'devlens-tree-title';
    title.textContent = 'Component Tree';

    const closeBtn = document.createElement('button');
    closeBtn.className = 'devlens-tree-close';
    ...
    searchWrap.className = 'devlens-tree-search';
    this.searchInput = document.createElement('input');
    this.searchInput.type = 'text';
    this.searchInput.placeholder = 'Filter components...';
    this.searchInput.className = 'devlens-tree-search-input';
    ...
    this.treeContainer.className = 'devlens-tree-content';
    this.container.appendChild(this.treeContainer);

    // Make panel draggable via resize handle
    const resizeHandle = document.createElement('div');
    resizeHandle.className = `devlens-tree-resize devlens-tree-resize-${this.options.position === 'right' ? 'left' : 'right'}`;
  ...
  selectElement(element: InspectedElement): void {
    ...
    const nodes = this.treeContainer.querySelectorAll('.devlens-tree-node');
    for (const node of nodes) {
      node.classList.remove('devlens-tree-node-selected');
      const path = node.getAttribute('data-path');
      if (path === `${element.filePath}:${element.line}`) {
        node.classList.add('devlens-tree-node-selected');
  ...
  private renderCurrentTree(): void {
    ...
    if (!this.currentTree) {
      ...
      empty.className = 'devlens-tree-empty';
  ...
  private renderNode(node: ComponentNode, parent: DocumentFragment | HTMLElement, depth: number): void {
    ...
    row.className = 'devlens-tree-node';
    ...
    arrow.className = 'devlens-tree-arrow';
    if (hasChildren) {
      ...
      arrow.classList.add('devlens-tree-arrow-expanded');
    }
    row.appendChild(arrow);

    // Component name
    const name = document.createElement('span');
    name.className = 'devlens-tree-name';
    ...
    if (this.options.showLineNumbers && node.line) {
      ...
      line.className = 'devlens-tree-line';
    ...
    if (this.options.showProps && node.props) {
      ...
      propsEl.className = 'devlens-tree-props';
    ...
    row.addEventListener('click', (e) => {
      ...
      if (hasChildren) {
        ...
        if (childContainer?.classList.contains('devlens-tree-children')) {
          const isCollapsed = childContainer.classList.toggle('devlens-tree-collapsed');
          arrow.classList.toggle('devlens-tree-arrow-collapsed', isCollapsed);
          arrow.classList.toggle('devlens-tree-arrow-expanded', !isCollapsed);
        }
      }

      // Update selected state
      this.treeContainer!.querySelectorAll('.devlens-tree-node-selected')
        .forEach((n) => n.classList.remove('devlens-tree-node-selected'));
      row.classList.add('devlens-tree-node-selected');
    });
    ...
    if (hasChildren) {
      ...
      childContainer.className = 'devlens-tree-children';
