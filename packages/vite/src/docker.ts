...
export function findComposeFile(root: string): string | null {
  const candidates = ['docker-compose.yaml', 'docker-compose.yml', 'compose.yaml', 'compose.yml'];
...
export function parsePathMappings(composeFile: string): Record<string, string> {
  ...
  for (const match of content.matchAll(volumeRe)) {
    ...
    const normalizedLocal = resolvedLocal.endsWith('/') ? resolvedLocal : resolvedLocal + '/';
